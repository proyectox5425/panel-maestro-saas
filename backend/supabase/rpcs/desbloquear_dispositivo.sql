-- RPC: desbloquear_dispositivo
CREATE OR REPLACE FUNCTION desbloquear_dispositivo(imei_input TEXT, usuario_input TEXT)
RETURNS JSON AS $$
DECLARE
  dispositivo_id UUID;
BEGIN
  ...
  -- Actualizar estado
  UPDATE dispositivos SET estado = 'activo' WHERE id = dispositivo_id;

  -- Registrar evento
  INSERT INTO eventos (
    imei, tipo, fecha, usuario, panel
  ) VALUES (
    imei_input, 'desbloqueo', NOW(), usuario_input, 'maestro'
  );
